...
describe ImportCustomerService do
  ...
  let(:bucket_key) { 'customer_template.csv' }
  ...
  describe '#execute' do
    context 'file upload success' do
      it 'enqueues CustomerImportJob' do
        expect { execution }.to have_enqueued_job.on_queue('import').with(bucket_key, triggered_user)
        expect(execution).to eq('Customers are queued to be import')
      end
    end
